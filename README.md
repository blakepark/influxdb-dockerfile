...
To create the image `cosmosio/influxdb`, execute the following command on influxdb-dockerfile folder:

    docker build -t cosmosio/influxdb .

You can now push new image to the registry:
    
    docker push cosmosio/influxdb
...
```docker run -d -p 8083:8083 -p 8084:8084 -e PRE_CREATE_DB="db1;db2;db3" cosmosio/influxdb:latest``` 
...
```docker run -d -p 8083:8083 -p 8086:8086 --expose 8090 --expose 8099 --expose 4444 -e UDP_DB="my_db" cosmosio/influxdb```
...
docker run -p 8083:8083 -p 8086:8086 --expose 8090 --expose 8099 \
  ...
  -d --name masterinflux cosmosio/influxdb
```
